...
const LINKANDROID_TASK_API_PROMPT = [
    ...
    'device.find(text=None, className=None, resourceId=None, description=None, packageName=None, index=None) -> UIElement：查找第一个元素；找不到会抛 ElementNotFoundError（含中文提示）。',
    'device.findOrNull(text=None, className=None, resourceId=None, description=None, packageName=None, index=None) -> UIElement|None：查找第一个元素；找不到返回 None，必须判空。',
    '⚠️ 禁止 device.find(...).click() 链式调用（元素不存在会直接抛错）。点击优先用 tapText/tapId/tapDesc/tapExists；必须用 find 时先判空: elem = device.findOrNull(...); if elem: elem.click()',
    ...
    const content: any[] = [
        {
            ...
            text: [
                ...
                '禁止 device.find(...).click() 链式调用：find() 找不到元素会抛异常。点击一律用 tapText/tapId/tapDesc/tapExists，或 findOrNull() 判空后再操作。',
                '',
...
const buildTaskAgentSystemPrompt = () =>
    [
        ...
        '禁止 device.find(...).click() 链式调用：find() 找不到元素会直接抛 ElementNotFoundError。',
        '点击元素一律用 tapText/tapId/tapDesc/tapExists；必须拿到元素对象时用 device.findOrNull(...) 判空后再操作。',
