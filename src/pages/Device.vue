...
onMounted(() => {
    ...
    testActionSet('device.connectWifi.show', () => connectWifiDialog.value?.show())
    testActionSet('device.connectWifi.fill', (data: any) => {
        if (connectWifiDialog.value) {
            connectWifiDialog.value.fillForm(data)
        }
    })
    testActionSet('device.connectWifi.submit', () => connectWifiDialog.value?.doSubmit())
})
